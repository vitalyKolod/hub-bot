...
При accept год считается от текущего expiresAt, если подписка ещё активна, иначе от now. Для продукта с groupId Telegram создаёт одноразовую invite link на 30 минут (`member_limit:1`). Для `add_member` вместо подписки создаётся deep link приглашения.
...
Owner покупает разовый `add_member`. После accept бот генерирует криптографический 8-символьный code и ссылку `?start=join_CODE`. Invite одноразовый, имеет status и expiresAt. Получатель проходит регистрацию при необходимости и принимает приглашение; затем добавляется в members, invite помечается used, owner получает уведомление.

Важное расхождение: service задаёт TTL 3 дня, а UI несколько раз говорит «24 часа». В новой реализации значение и текст должны исходить из одной константы.
...
- Invite acceptance состоит из отдельных операций add member + consume invite и подвержен гонке; нужен atomic transaction/conditional update.
...
- TTL приглашения: код 3 дня, тексты 24 часа.
